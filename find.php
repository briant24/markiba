  ...
  <div class="happy-clients section">
    <div class="container">
      <div class="row">
        ...
        $param = $_POST['keyword'];
        $sql = "SELECT buku.* , ulasan.rating 
        FROM buku
        LEFT JOIN ulasan ON buku.id_buku = ulasan.id_buku
        WHERE buku.nama_penulis LIKE '%$param%' OR buku.judul_buku LIKE '%$param%'";
        ...
        if (mysqli_num_rows($result) > 0) {
          ...
          // Pagination links
          $sql = "SELECT COUNT(buku.id_buku) AS total FROM buku
          LEFT JOIN ulasan ON buku.id_buku = ulasan.id_buku
          WHERE nama_penulis LIKE '$param' OR judul_buku LIKE '$param'";
          $result = mysqli_query($conn, $sql);
